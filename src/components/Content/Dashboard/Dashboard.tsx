...
const Dashboard = (props: Props) => {
    ...
    //student
    // useEffect(() => {
    //     fetch(`http://localhost:8080/getUserByClassID/${idClass}`)
    //     .then(
    //         (responses) => {
    ...
    // const GetStudentByClassID = (value) => {
    //    console.log(value.typeO)
    //         fetchStudent.fetchDataStudent(e).then(
    //             (response) => {
    //                 console.log(response)
    ...
    return (
        <div>
            <div className="site-card-wrapper" style={{ marginBottom: "3rem", padding: "0.5rem", background: "#ddd" }}>
                <Row gutter={18}>
                    <Col span={8} style={{}}>
                        <Card title="Students Count" style={{ fontWeight: "700" }} bordered={true}>
                            {students.length}
                        </Card>
                    </Col>
                    <Col span={4}>
                        <Card title="Excellent" style={{ fontWeight: "700" }} bordered={true}>
                            1
                        </Card>
                    </Col>
                    <Col span={4}>
                    ...
                    <Col span={4}>
                        <Card title="Below Average" style={{ fontWeight: "700" }} cover bordered={true}>
                            1
                        </Card>
                    </Col>
                </Row>
            ...
            <div className="chart" style={{ display: "flex", justifyContent: "space-around", marginBottom: "2rem" }}>
            ...
            <div className='filter' style={{ display: "flex" }}>
                <div className='filter-subject' style={{ padding: "1rem 1rem 0.5rem 0" }}>
                    ...
                    >
                        {subject.map((sub, index) => (
                            <Option key={index} value={sub.id}>{sub.id} - {sub.name}</Option>
                        ))}
                    </Select>
                </div>
                <div className="filter-class" style={{ padding: "1rem 1rem 0.5rem 0" }}>
                    ...
                    >
                        {classes.map((classes, index) => (
                            <Option key={index} value={classes.classID}>{classes.classID} - {classes.className}</Option>
                        ))}
                    </Select>
                </div>
            </div>
